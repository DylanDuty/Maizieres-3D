...
import {ARCH_COLORS,ARCH_LABELS} from './architecture.js';
import './style.css';
...
async function start(){
  ...
  // V2.3: architectural profiles (roof type, ridge, heights, colour family) previewed with ?architecture=1, coloured by confidence with ?diagnostic=architecture.
  // The default map keeps the V2.2 rendering until the V2.3 profiles are validated.
  const archDiagnostic=params.get('diagnostic')==='architecture',archMode=v2Mode&&(archDiagnostic||params.get('architecture')==='1');
  ...
  if(archDiagnostic&&architecture){const legend=document.createElement('div');legend.id='diagnostic-legend';const st=architecture.metadata.stats,n=k=>architecture.buildings.filter(e=>e.confidenceOverall===k).length;
   legend.innerHTML=Object.entries(ARCH_LABELS).map(([k,v])=>`<span><i style="background:${ARCH_COLORS[k]}"></i>${v} · ${n(k)}</span>`).join('')+`<span>Profils V2.3 : ${architecture.buildings.length} · toit A/B ${st.roofTypeAB} · faîtage A/B ${st.ridgeAB} · hauteur officielle ou mesurée ${st.heightOfficialOrMeasured}</span>`;document.body.appendChild(legend);}
