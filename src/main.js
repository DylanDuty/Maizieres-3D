...
async function start(){
  ...
  const responses=await Promise.all(['maizieres.geojson','commune.geojson','building-enrichment.json','ign-landscape.geojson','named-zones.geojson'].map(name=>fetch(`${import.meta.env.BASE_URL}data/${name}`)));
  if(responses.some(r=>!r.ok))throw Error('Les données locales sont introuvables.');
  const [data,boundary,enrichment,ignLandscape,namedZones]=await Promise.all(responses.map(r=>r.json()));
  ...
  const scene=new THREE.Scene();scene.background=new THREE.Color('#c4e1ec');scene.fog=new THREE.Fog('#c4e1ec',7200,15000);
  ...
  const sun=new THREE.DirectionalLight('#ffedc7',2.25);sun.position.set(-1300,2200,-900);sun.castShadow=true;sun.shadow.mapSize.set(quality.shadowSize,quality.shadowSize);Object.assign(sun.shadow.camera,{left:-2200,right:2200,top:2200,bottom:-2200,near:50,far:6000});sun.shadow.bias=-.00012;sun.shadow.normalBias=1.2;scene.add(sun,new THREE.HemisphereLight('#c8dce9','#c7b58e',.85));
  ...
  const catalogue=createCatalogue(data,enrichment,project,extent,terrain.buildings,namedZones);
  const selection=installSelection({scene,camera,canvas:renderer.domElement,catalogue,meshes:buildings.pickMeshes,invalidate});
  const labelItems=[...buildings.landmarks,...terrain.landmarks];
  for(const r of catalogue.records.filter(r=>r.type==='point'&&r.kind==='Lieu-dit / secteur'))if(!labelItems.some(l=>l.name===r.name))labelItems.push({id:r.id,name:r.name,position:r.position,major:r.major,local:!r.major});
  ...
  const labels=labelItems.map(item=>{const el=document.createElement('button');el.type='button';el.className=`map-label${item.major?' major':''}`;el.textContent=item.name;selection.bindLabel(el,item);document.querySelector('#labels-layer').appendChild(el);return {...item,el,vector:new THREE.Vector3(...item.position)};});
  ...
  function reset(){controls.enableDamping=false;controls.update();controls.target.copy(homeTarget);const portrait=innerWidth/innerHeight<1;camera.position.copy(homeTarget).add(new THREE.Vector3(240,portrait?3300:1900,portrait?3900:2350));controls.update();controls.enableDamping=true;invalidate();}
