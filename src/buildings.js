...
export function buildBuildings(scene,items,enrichment={buildings:{}}){
 ...
 let count=0;const info=new Map(),landmarks=[],stats={ignWallHeights:0,ignRoofHeights:0,knownFloors:0,knownRoofMaterials:0,roofHeightClamps:0,categories:{}};
 for(const {poly,t,id} of items){if(area(poly[0])<3||area(poly[0])>100000)continue;const extra=enrichment.buildings[id]||{},p=buildingProfile(t,poly,extra,id);if(!p)continue;
  ...
  info.set(id,{kind:p.kind,knownUsage:p.knownUsage,usage:extra.usage&&extra.usage!=='Indifférencié'?extra.usage:null,floors:extra.floors||Number.parseInt(t['building:levels'])||null,wallHeight:p.wallHeight,heightSource:p.heightSource,maxHeight,light:t.wall==='no'||extra.lightConstruction===true,ign:!!extra.ignId,poly});
 ...
 for(const [i,batch] of batches.entries()){const mesh=batch.mesh(scene,i!==2);if(mesh){mesh.receiveShadow=false;mesh.userData.featureRanges=ranges[i];pickMeshes.push(mesh);}}return {count,landmarks,stats,pickMeshes,info};
}
