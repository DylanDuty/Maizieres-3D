...
export function buildingItems(fc,project,extent){
 ...
 for(const f of fc.features){const p=f.properties,parts=polygons(f,project);
  parts.forEach((raw,k)=>{stats.parts++;const b=bounds(raw[0]);if(!inside([(b.minX+b.maxX)/2,(b.minZ+b.maxZ)/2])){stats.outsideExtent++;return;}
   ...
   items.push({poly,t,id:parts.length>1?`${p.id}#${k+1}`:p.id,featureId:p.id,provenance:p.provenance,source:p.source,validation:p.validation||null,rnb:p.rnb,inCommune:p.inCommune,audit:p.audit||null,manualInfo:p.manualGeometry?{category:p.categoryLabel,note:p.categoryNote,uncertainty:p.uncertainty,missingId:p.originalMissingId}:null,extra});
  });
 }
 return {items,stats};
}
