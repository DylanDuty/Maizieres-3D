...
// V1.5: the scene is built from the unified reference; the V1.1 enrichment remains a checked OSM→IGN record.
const reference=buildingItems(read('public/data/buildings.geojson'),project,extent);
...
const scene=new THREE.Scene(),result=buildBuildings(scene,reference.items,enrichment);assert.equal(result.rejected.length,0,'Bâtiment rejeté au rendu : '+JSON.stringify(result.rejected.slice(0,3)));assert.equal(result.count,reference.items.length);let maxY=0,vertices=0;
...
console.log(JSON.stringify({result:'OK',sourceHashes:'unchanged',courtyardAreaM2:renderedArea,matched,buildings:result.count,referenceItems:reference.stats,stats:result.stats,maxHeightM:+maxY.toFixed(2),buildingTriangles:vertices/3},null,2));
