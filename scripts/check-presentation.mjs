...
import {projection,polygons,area} from '../src/geo.js';
...
const data=read('public/data/maizieres.geojson'),enrichment=read('public/data/building-enrichment.json'),project=projection(data.metadata.origin),adjustments=[];
for(const f of data.features.filter(f=>f.properties.building))for(const poly of polygons(f,project)){
 if(area(poly[0])<3)continue;const e=enrichment.buildings[f.id]||{},p=buildingProfile(f.properties,poly,e,f.id);if(!p)continue;
 if(e.wallHeight&&!f.properties.height)assert.equal(p.wallHeight,e.wallHeight,'Hauteur IGN de mur modifiée');
 if(p.roofHeightClamped)adjustments.push({osmId:f.id,ignId:e.ignId,widthM:+(p.axis.max-p.axis.min).toFixed(2),sourceRoofHeight:p.sourceRoofHeight,renderedRoofHeight:+p.roofHeight.toFixed(2),floors:e.floors,wallHeight:p.wallHeight});
}
